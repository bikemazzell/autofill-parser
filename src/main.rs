...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    log_message(&format!("Verbose mode: {}", args.verbose)); // Log verbose status

    let input_path = Path::new(&args.input);
    if !input_path.is_dir() {
        let err_msg = format!("Input path is not a directory: {}", args.input);
        log_message(&err_msg); // This will print to stderr if verbose, always to log
    ...
    println!("Processing files from: {}", input_file_pattern); // General progress
        ...
        match entry {
            Ok(path) => {
                if path.is_file() {
                    ...
                    if args.verbose { // Only print if verbose
                    ...
                    for (line_number, line_result) in reader.lines().enumerate() {
                        ...
                        if line.trim().is_empty() {
                            // Potentially verbose: log_message(&format!("Skipping empty line {} in {:?}", line_number + 1, path.display()));
                            continue;
                        }
